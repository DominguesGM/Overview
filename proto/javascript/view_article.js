  ...
  function getCommentHtml(commentId, commentDate, content, score, postedBy, firstName, lastName, path, vote, report){
    ...
    if(contributorAccess){
      commentHtml += '<div class="comment-vote"><a onclick="upvoteComment(' + commentId + ')"><i id="comment-up-vote-' + commentId + '"';
    ...
    commentHtml += content;
    commentHtml += '<p><small id="comment-score-' + commentId + '" class="text-muted">' + score + (score != 1 ? ' pontos' : ' ponto') + '</small></p>';
